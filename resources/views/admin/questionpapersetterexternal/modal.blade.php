  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="addModalLabel">Add Question Paper Setter External</h5>
  ...
  <option  data-item_des="{{$tech->designation}}" data-item_dep="{{$tech->department->department_name}}" data-item_add="{{$tech->address}}" value="{{$tech->id}}">{{$tech->name}} </option>
    @endforeach
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary add_questionpapersetterexternal">Add Question Paper Setter External</button>
      </div>
    </div>
  </div>
</form>
